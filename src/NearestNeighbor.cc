...
void NearestNeighbor::CacheReset() {
	...
	mNeighborhoodCache.resize(mMinHashEncoder.GetLoadedInstances());
	mNeighborhoodCacheExt.resize(mMinHashEncoder.GetLoadedInstances());
	mNeighborhoodCacheInfo.resize(mMinHashEncoder.GetLoadedInstances());
}
...
//	ProgressBar pb;
//	vector<vector<unsigned> > new_neighborhood_cache(mMinHashEncoder.GetLoadedInstances());
//#ifdef USEMULTITHREAD
//#pragma omp parallel for schedule(dynamic)
//#endif
//	for (unsigned i = 0; i < mMinHashEncoder.GetLoadedInstances(); ++i) {
